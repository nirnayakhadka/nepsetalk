...
import { updateBlog, getBlogById, getCategories } from '../services/adminApi';

// ── ImageUploader ─────────────────────────────────────────────────────────────
const ImageUploader = ({ onChange, initialImage = '' }) => {
...
const EditBlog = () => {
  ...
  const [formData,   setFormData]   = useState({
    ...
    category_id: '', status: 'draft', read_time: '', thumbnail: '',
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (blog.thumbnail || blog.image) {
          setImageState({ file: null, url: blog.thumbnail || blog.image });
        }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0f172a 0%,#0c1a2e 50%,#0f172a 100%)', fontFamily: "'DM Sans',sans-serif", display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
        <div style={{ width: 36, height: 36, border: '3px solid rgba(6,182,212,.2)', borderTopColor: '#06b6d4', borderRadius: '50%', animation: 'spin .7s linear infinite' }} />
        <p style={{ color: '#7c839e', fontSize: 14 }}>Loading blog…</p>
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0f172a 0%,#0c1a2e 50%,#0f172a 100%)', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '36px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
          ...
          <div>
            <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: 26, fontWeight: 800, color: '#fff', margin: 0 }}>Edit Blog Post</h1>
            <p style={{ color: '#64748b', marginTop: 4, fontSize: 14 }}>Update article details</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={cardCls}>
            ...
            <ImageUploader onChange={setImageState} initialImage={formData.thumbnail} />
          </div>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <button type="submit" disabled={saving} style={{ flex: 2, padding: '13px', background: saving ? 'rgba(8,145,178,.4)' : 'linear-gradient(135deg,#0891b2,#0e7490)', border: 'none', borderRadius: 12, color: '#fff', fontWeight: 700, fontSize: 14, cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'opacity .15s' }}>
              {saving
                ? <><div style={{ width: 18, height: 18, border: '2px solid rgba(255,255,255,.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin .7s linear infinite' }} />Updating…</>
                : <><Plus size={18} />Update Blog Post</>}
            </button>
          </div>
